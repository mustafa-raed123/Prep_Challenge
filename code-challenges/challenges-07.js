...
const cvFormatter = (arr) => {
    let formatcv =[];
    let ar = arr.filter(index => index.yearsOfExperience >1);
    console.log(ar)
       formatcv = ar.map(element => {   
               var fullName = (element.firstName && element.lastName ? `${element.firstName} ${element.lastName}`: element.firstName || element.lastName);
               console.log(fullName);
                  var cv ={};
                  cv["fullName"] = fullName;
                  cv["tech"] = element.tech;
