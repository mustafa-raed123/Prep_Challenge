...
const getStudents = (arr) => {
  let arrofstudent = [];
  let ss = {};
  for (let std = 0 ; std < arr.length ; std++ ) {
  courses.forEach(element => {
    for (let s = 0; s < element.Students.length; s++) {
        if(element.Students[s] ==arr[std] ){        
            ss ={
              "Student": `${arr[std]}`,
              "course": `${element.course}`
            }
            arrofstudent.push(ss)     
          }  
        }   
      });
    }
