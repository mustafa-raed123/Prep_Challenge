...
const employeesBonus = (arr) => {
    arr.forEach(element => {
        let num = (parseInt(element.salary.replace("$","")));
        let salary =0 ;
          salary= element.workHours > 8 ? salary = num + 100: salary = num + 50;
    ...
    mouseArray.forEach(mouse => {
      keyBoardArray.forEach(Keyboard =>{
       if((mouse + Keyboard) <= budget){
        bud.push(mouse + Keyboard)
       }
      })  
    });
    let max = bud.reduce((acc,current)=> Math.max(acc,current))
    return max;
}
// -------------------------------------------------------------------------------------------------------
