...
const filterLinks = (str) => {
  const s = str.split('>')
  const q = s.filter(ele => ele.includes("href")).join("")
  const link = q.split("http://").filter(ele => ele.includes("www")).join("")
  const links = link.split("\"")
  return links.filter(ele => ele.includes("www")).join("")
